...
tools: [execute, read]
argument-hint: "Optional: resource group, subscription, poll interval seconds, timeout minutes"
---
...
- If Azure login/context is missing, report it and stop.
...
3. Start all discovered VMs:
   - `az vm start --ids <vm-id>` for each VM.
4. Monitor power state in a polling loop until all are running or timeout:
