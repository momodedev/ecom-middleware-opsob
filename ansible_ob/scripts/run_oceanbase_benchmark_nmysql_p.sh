#!/bin/bash
###############################################################################
# run_oceanbase_benchmark_nmysql_p.sh
#
# OceanBase sysbench benchmark script via OBProxy, aligned with
# run_oceanbase_benchmark_nmysql.sh test specification:
#   - 90 tables x 500,000 rows (~10 GB+)
#   - 300 s run time per test case
#   - 120 s warmup (50 threads, read_only)
#   - Thread ladder: 20, 50, 100, 200
#   - 30 s cool-down between cases
#
# OBProxy benchmark workloads:
...
#
# Example:
#   ./run_oceanbase_benchmark_nmysql_p.sh d8s_v6_rocky_proxy 172.17.1.7 \
#       root@sbtest_tenant#ob_cluster 'OceanBase#!123' sbtest \
#       "172.17.1.5 172.17.1.6 172.17.1.7" 2883
###############################################################################
set -uo pipefail
...
# Validate OBProxy connectivity and DB creation path
...
if [ "$SKIP_PREPARE" = "1" ]; then
  ...
  echo "[1/4] Preparing test data (${TABLES} tables x ${TABLE_SIZE} rows, ~10 GB+)..."
